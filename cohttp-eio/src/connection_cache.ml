type connection = Eio.Flow.two_way_ty Eio.Resource.t
...
let tcp_address ~net uri =
  ...
  with
  | ip :: _ -> ip
  | [] -> Fmt.failwith "failed to resolve hostname %a" Uri.pp uri

let connect ~sw ~net ~https uri =
...
module Make_no_cache = struct
  ...
  let call (t : t) =
    let https = t.https in
    fun ?headers ?body ?absolute_form:_ ?(chunked = false)
        (meth : Http.Method.t) (uri : Uri.t) ~sw ~net ->
      let socket = connect ~sw ~net ~https uri in
      let body_length =
      ...
      let () =
        ...
        Io.Request.write ~flush:false
          (fun writer ->
            ...
            | None -> ()
            | Some body ->
                Utils.flow_to_writer body writer Io.Request.write_body)
          request output
      in
